...
<template>
  <div id="body">
    ...
    <div class="flex justify-center">
      <FormKit type="form" :actions="false">
        <FormKit type="multi-step" tab-style="progress" :allow-incomplete="false">
          <!-- Destination: start -->
          <FormKit type="step" name="Destination" :nextLabel="nextStepDisabled ? `Next` : 'Please enter a valid city'"
            nextAttrs:nextStepDisabled>
            <!-- collect name, email, and company info -->
            <FormKit v-model="town" type="text" label="Destination" validation="required" @blur="checkCityExists(town)"
              placeholder="e.g. Singapore" help="City Name" />
            <FormKit v-model="sliderValue" type="number" label="Duration (days)" validation="max:3 | min:1" value=""
              min="1" max="3" help="between 1 & 3 days" />
          ...
          <FormKit type="step" name="Preferences">
            ...
            <FormKit type="radio" label="Outdoors or Indoors?" help="Which type of setting do you prefer?"
              validation="required" v-model="outgoing" :options="[
                'Outdoors',
                'Indoors',
                'I\'m fine with either',
              ]" />
            <FormKit v-model="interestsoptions" type="checkbox" label="Places of Interest (optional)"
              help="Any specific places you'd like to visit?" :options="[
                'Museums',
                'Shopping Malls',
                'Gardens',
              ]" />

            <FormKit v-model="transport" type="radio" label="Mode of Transportation"
            ...
            <FormKit v-model="starttime" type="time" label="What time do you want to start your day?"
              validation="required" value="09:00" min="08:00" max="12:00" />
            <!-- <p>Time: {{ starttime }}</p> -->
          ...
          <FormKit type="step" name="Let's go!">
            ...
            <br>
            <template #stepNext>
              <FormKit type="button" @click="checkempty" label="Generate an Itinerary for me!" />
            </template>
            <!-- Generate: end -->
          </FormKit>
        </FormKit>
      ...
      <div v-if="final_activities.length > 0" class="">
        <div class="flex flex-col justify-center items-center">
          <div v-if="showAlert" class="alert alert-success w-2/3">
            <svg xmlns="https://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
          ...
          <div v-if="!isLoading">
            <button
              class="transition ease-in-out delay-150 bg-blue-500 hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 duration-300 btn mt-7 mr-2"
              @click="saveItinerary">Save Itinerary</button>
          </div>
        </div>
        <!-- start of iti  -->
    ...
    <div v-if="customactivitiesandtime"></div>
  </div>
</template>
